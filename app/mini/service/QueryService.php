...
class QueryService extends Service
{
    ...
    public static function record(int $userId, string $platform, string $inputUrl, array $result = [], string $failReason = ''): array
    {
        ...
        $status  = !empty($result) ? MiniUserQuery::STATUS_SUCCESS : MiniUserQuery::STATUS_FAIL;
        $queryId = 0;
        ...
        try {
            ...
            if (!$isVip && $costScore > 0) {
                ...
                MiniUserScoreLog::record(
                    ...
                    -$costScore,
                    $newBalance,
                    MiniUserScoreLog::SOURCE_QUERY_COST,
                    0,
                    '查询扣除：' . $platform
                );
            }

            $query = MiniUserQuery::mk();
            $query->save([
                'user_id'      => $userId,
                'platform'     => $platform,
                'input_url'    => $inputUrl,
                'query_result' => $status === MiniUserQuery::STATUS_SUCCESS ? json_encode($result, JSON_UNESCAPED_UNICODE) : '',
