...
class Vip extends Controller
{
    ...
    protected function _form_filter(array &$data): void
    {
        if ($this->request->isPost()) {
            ...
            if (empty($data['id'])) {
                ...
                $exists = MiniCardVip::mk()->where(['card_code' => $data['card_code']])->count();
                if ($exists > 0) {
    ...
    public function apply(): void
    {
        if ($this->request->isPost()) {
            ...
            $card = MiniCardVip::mk()->where(['id' => $data['id']])->findOrEmpty();
            if (!$card->isExists()) {
                $this->error('卡密不存在！');
            }
