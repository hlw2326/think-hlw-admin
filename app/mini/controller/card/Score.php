...
use app\mini\service\CardUseService;
...
class Score extends Controller
{
    ...
    {
        MiniCardScore::mQuery()->layTable(function () {
            $this->title = '积分卡密';
    ...
    protected function _form_filter(array &$data): void
    {
        if ($this->request->isPost()) {
            ...
            if (empty($data['id'])) {
                ...
                $exists = MiniCardScore::mk()->where(['card_code' => $data['card_code']])->count();
                if ($exists > 0) {
    ...
    {
        MiniCardScore::mSave($this->_vali([
    ...
    private function _genCode(): string
    {
        ...
        } while (MiniCardScore::mk()->where(['card_code' => $code])->count() > 0);
        return $code;
    }
}
